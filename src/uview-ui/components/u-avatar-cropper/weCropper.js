...
(function (global, factory) {
  ...
  let device = void 0;
  let TOUCH_STATE = ["touchstarted", "touchmoved", "touchended"];
  ...
  let ref$1 = getDevice();
  let windowWidth = ref$1.windowWidth;
  ...
  function prepare() {
    ...
    self.attachPage = function () {
      ...
      // 获取到当前page上下文
      let pageContext = pages[pages.length - 1];
  ...
  let tools = createCommonjsModule(function (module, exports) {
    /**
     * String type check
     */
  ...
  let tools_7 = tools.isFunc;
  let tools_10 = tools.isPlainObject;

  let EVENT_TYPE = ["ready", "beforeImageLoad", "beforeDraw", "imageLoad"];
  ...
  let getImageInfo = wxPromise(uni.getImageInfo);

  let canvasToTempFilePath = wxPromise(uni.canvasToTempFilePath);

  let base64 = createCommonjsModule(function (module, exports) {
    /*! http://mths.be/base64 v0.1.0 by @mathias | MIT license */
    (function (root) {
      // Detect free variables `exports`.
      let freeExports = "object" == "object" && exports;
      ...
      // it as `root`.
      let freeGlobal = typeof commonjsGlobal == "object" && commonjsGlobal;
      if (
  ...
  function methods() {
    ...
    let boundWidth = self.width; // 裁剪框默认宽度，即整个画布宽度
    let boundHeight = self.height; // 裁剪框默认高度，即整个画布高度

    let id = self.id;
    let targetId = self.targetId;
    let pixelRatio = self.pixelRatio;
    ...
    self.getCropperImage = function (opt, fn) {
      ...
      return task()
        .then(function () {
          ...
          let arg = canvasOptions.componentContext
   ...
   * @returns {*}
   */
  let getNewScale = function (oldScale, oldDistance, zoom, touch0, touch1) {
  ...
  function update() {
    ...
    self.__oneTouchMove = function (touch) {
      ...
      let imgLeft = Math.round(self.rectX + xMove);
      let imgTop = Math.round(self.rectY + yMove);
    ...
    self.__twoTouchMove = function (touch0, touch1) {
      let oldScale = self.oldScale;
      let oldDistance = self.oldDistance;
      ...
      let imgLeft = Math.round(self.touchX1 - self.scaleWidth / 2);
      let imgTop = Math.round(self.touchY1 - self.scaleHeight / 2);
  ...
  let WeCropper = function WeCropper(params) {
    let self = this;
    let _default = {};
